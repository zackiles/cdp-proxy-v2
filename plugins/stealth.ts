...
/** The display headless does not have: a monitor bigger than the viewport. */
const SCREEN = { width: 1920, height: 1080, scale: 1 }

/**
 * Height of the tab strip plus toolbar, the gap between `outerHeight` and
 * `innerHeight` that every real Chrome window has and a headless one does not.
 */
const BROWSER_CHROME = 88

export interface StealthOptions {
  /** Override the spoofed User-Agent; defaults to the browser's own, de-headlessed. */
  userAgent?: string
  /**
   * `Accept-Language`, and with it `navigator.languages`, which headless otherwise
   * reports as the single entry `['en-US']`. Worth setting to match the country of
   * whatever network proxy the session goes out through. Give it plain
   * comma-separated tags rather than a real header's q-weights — see `defaults`.
   */
  acceptLanguage?: string
  /** The display to claim. Replaced wholesale, not merged field by field. */
...
>({
  name: 'stealth',
  priority: 100,
  // The q-weights of a real Accept-Language header would be parsed as languages
  // in their own right, putting a literal "en;q=0.9" in `navigator.languages`.
  defaults: { acceptLanguage: 'en-US,en', screen: SCREEN },
  setup(cfg, ctx) {
    /**
    ...
    return {
      onRequest(msg) {
        // Playwright pins the screen to the viewport it was asked for, so headless
        // reports `screen.width === innerWidth`, which no real monitor does. Widen
        // the world around the viewport without touching the viewport itself.
        if (msg.method === 'Emulation.setDeviceMetricsOverride') {
          const params = msg.params as unknown as {
            screenWidth?: number
            screenHeight?: number
            deviceScaleFactor?: number
          }
          params.screenWidth = cfg.screen!.width
          params.screenHeight = cfg.screen!.height
          params.deviceScaleFactor = cfg.screen!.scale
          return msg
        }

        // Playwright then sizes the window to the viewport exactly, leaving
        // `outerHeight === innerHeight` — a window with no tab strip and no
        // toolbar. Give those back the height they occupy in a real one.
        if (msg.method === 'Browser.setWindowBounds') {
          const bounds = (msg.params as { bounds?: { height?: number } })
            ?.bounds
          if (bounds?.height !== undefined) {
            bounds.height += BROWSER_CHROME
          }
          return msg
        }
