 ...
 */
async function killProcessOnPortByName(port: string | number, pattern: RegExp): Promise<void> {
  ...
  const isWSL = Deno.build.os === "linux" && 
    (await Deno.readTextFile("/proc/version").catch(() => "")).includes("Microsoft");
  ...
  const getPids = async (): Promise<string[]> => {
    try {
      if (Deno.build.os === "windows" && !isWSL) {
        ...
        const out = new TextDecoder().decode(await new Response(netstat.stdout).arrayBuffer());
  ...
  try {
    ...
    if (Deno.build.os === "windows" && !isWSL) {
      try {
        ...
        const output = await runCommand({ cmd: "tasklist", args: ["/FO", "CSV", "/NH"] });
        const processMap = output.split("\n")
          .map(line => line.trim())
          .filter(Boolean)
        ...
        const pidsToKill = pids.filter(pid => 
          processMap[pid] && pattern.test(processMap[pid]));
        ...
        for (const pid of pids) {
          ...
          for (let retryCount = 0; retryCount < MAX_RETRY_COUNT; retryCount++) {
            ...
            await new Promise(resolve => 
              setTimeout(resolve, Math.min(MAX_BACKOFF_MS, 2 ** retryCount * 10)));
          }
        }
      } catch (error: unknown) {
        const errorMessage = error instanceof Error ? error.message : String(error);
