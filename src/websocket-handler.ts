...
/**
 * Handles WebSocket connections for Chrome DevTools Protocol proxy
 */
export class WebSocketHandler implements HandlerInterface {
  private browserHost: string
  private browserPort: number

  /**
   * Creates a new WebSocketHandler instance
   * @param options Configuration options containing browser host and port
   */
  ...
  /**
   * Handles a WebSocket connection request by creating a transparent proxy
   ...
   */
  async handle(clientRequest: Request): Promise<Response> {
    const messageLog: WSMessage[] = []

    // Construct the browser's WebSocket debugger URL using the request path but browser host/port
    const requestUrl = new URL(clientRequest.url)
    const webSocketDebuggerUrl = `${requestUrl.protocol === 'https:' ? 'wss' : 'ws'}://${this.browserHost}:${this.browserPort}${requestUrl.pathname}${requestUrl.search}`
    ...
    browserRequestHeaders.set(
      'host',
      `${browserUrl.hostname}:${browserUrl.port}`,
    )
